...
def set_loader(args):
    ...
    if args.dataset == 'RAF':
        ...
        AU_train_loader = data.DataLoader(AU_train_dataset,batch_size=int(args.batch_size), shuffle=True,num_workers=args.num_workers, pin_memory=True)
        AU_test_dataset =  RAFAU_100.load_RAFAU(AU_test_list_file,AU_img_folder_path,transform=val_transform,phase="train")
        AU_test_loader = data.DataLoader(AU_test_dataset,batch_size=int(args.batch_size/3), shuffle=True,num_workers=args.num_workers, pin_memory=True)

        TestlabelPath = parameter.FER_TestlabelPath
        TrainlabelPath = parameter.FER_TrainlabelPath
        FER_img_folder_path =parameter.FER_img_folder_path

        TrainDataset=RAFDB_100.load_RAFDB(TrainlabelPath,FER_img_folder_path,transform=train_transform,index=11)
        TrainLoader=data.DataLoader(TrainDataset,batch_size=args.batch_size,num_workers=args.num_workers,shuffle=True)
        
        TestDataset=RAFDB_100.load_RAFDB(TestlabelPath,FER_img_folder_path,transform=val_transform,index=9)
        TestLoader=data.DataLoader(TestDataset,batch_size=args.batch_size*3,num_workers=args.num_workers,shuffle=False)
...
def set_model(args):
    ...
    model =IR_RVT_AU_plus_patch.IR50_ViT(ir_50_pth=pthpath,num_AU_patch=args.AU_patch)
