...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
        ...
        self.norm_AU_up = nn.LayerNorm(embed_dim, eps=1e-6)
        self.norm_AU_low = nn.LayerNorm(embed_dim, eps=1e-6)
        self.upperAU_head= nn.Linear(embed_dim, upperAU_cls)
        self.lowerAU_head= nn.Linear(embed_dim, lowerAU_cls)
    ...
    def AUBranch(self,AU_x):
        upper_AU=AU_x[:,:, :8 ,:]
        lower_AU=AU_x[:,:, 6: ,:]
        upperAU_feat = self.norm_AU_up(self.gap_upperAU(upper_AU).squeeze())
        lowerAU_feat = self.norm_AU_low(self.gap_lowerAU(lower_AU).squeeze())
        return upperAU_feat,lowerAU_feat
    ...
    def forward_features(self, x):
        ...
        x = self.layer2(x)
        x = self.layer3(x)
        upperAU_feat,lowerAU_feat = self.AUBranch(x)
        x = self.layer4(x)
        return x,upperAU_feat,lowerAU_feat

    def forward(self, x):
        x,upperAU_feat,lowerAU_feat = self.forward_features(x)
        upperAU = self.upperAU_head(upperAU_feat)
        lowerAU = self.lowerAU_head(lowerAU_feat)
        AU_output=torch.cat((upperAU, lowerAU), dim=1)
...
@register_model
def resnet50(pretrained=False,upperAU_cls=6,lowerAU_cls=10, **kwargs):
    """Constructs a ResNet-50 model.
    """
    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3],upperAU_cls=upperAU_cls,lowerAU_cls=lowerAU_cls,  **kwargs)
