...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,num_classes=7, num_patches=196, embed_dim=256, depth=6,AUspv_layer=4,AU_num_classes=21,
                 num_heads=16, mlp_ratio=4., qkv_bias=True,distilled=False,
                 drop_rate=0.0, attn_drop_rate=0., drop_path_rate=0., norm_layer=None,
        ...
        self.blocks_1 = nn.Sequential(*[
            ...
            for i in range(AUspv_layer)])
        self.blocks_2 = nn.Sequential(*[
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer)
            for i in range(depth-AUspv_layer)])
...
class IR50_ViT(nn.Module):
    ...
    def forward(self,x):
        ...
        FER_cls, AU_supervise = self.vit(x)                   # ViT
        return FER_cls,AU_supervise