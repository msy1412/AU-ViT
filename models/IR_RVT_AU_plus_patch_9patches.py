...
class PoolingTransformer(nn.Module):
    def __init__(self, base_dims, depth, heads,
                 ...
                 attn_drop_rate=.0, drop_rate=.0, drop_path_rate=.0, use_mask=False, masked_block=None,num_AU_patch=7):
        ...
        if num_AU_patch==9:
            ...
            self.gap_AU31  = nn.AdaptiveAvgPool2d(1)
            self.gap_AU32  = nn.AdaptiveAvgPool2d(1)
            self.gap_AU33  = nn.AdaptiveAvgPool2d(1)
            ...
            self.norm_AU31  = nn.LayerNorm(self.embed_dim, eps=1e-6)
            self.norm_AU32  = nn.LayerNorm(self.embed_dim, eps=1e-6)
            self.norm_AU33  = nn.LayerNorm(self.embed_dim, eps=1e-6)
            ...
            self.head_AU31= nn.Linear(self.embed_dim, 14)
            self.head_AU32= nn.Linear(self.embed_dim, 14)
            self.head_AU33= nn.Linear(self.embed_dim, 14)
        elif num_AU_patch==7:
            self.gap_AU11 = nn.AdaptiveAvgPool2d(1)
            self.gap_AU12 = nn.AdaptiveAvgPool2d(1)
            self.gap_AU13 = nn.AdaptiveAvgPool2d(1)
            ...
            self.lowerAU_head= nn.Linear(512, 14)
        elif num_AU_patch==1:
    ...
    def AUBranch(self,AU_x):
        ...
        if self.num_AU_patch==9:
            ...
            AU31 =self.head_AU31(self.norm_AU31(self.gap_AU31(AU31).squeeze()))
            AU32 =self.head_AU32(self.norm_AU32(self.gap_AU32(AU32).squeeze()))
            AU33 =self.head_AU33(self.norm_AU33(self.gap_AU33(AU33).squeeze()))
            AU1257=torch.maximum(AU11,AU13)
            AU6=torch.maximum(AU21,AU23)
            AU3=torch.maximum(AU31,AU32)
            AU3=torch.maximum(AU3,AU33)
            AU_all=torch.cat((AU1257[:,:2],AU12,AU1257[:,2].view(B,-1),AU6,AU1257[:,3].view(B,-1),AU22,AU3),dim=1)
 
        elif self.num_AU_patch==7:
            AU11=AU_x[:,:, :7 , :7 ]
            AU12=AU_x[:,:, :7 ,4:10]
            AU13=AU_x[:,:, :7 ,7:  ]
            AU21=AU_x[:,:,5:12, :6 ]
