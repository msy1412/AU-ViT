...
class PoolingTransformer(nn.Module):
    def __init__(self, base_dims, depth, heads,
        ...
        # self.norm_AU_up = nn.LayerNorm(base_dims[-1] * heads[-1], eps=1e-6)
        # self.norm_AU_low = nn.LayerNorm(base_dims[-1] * heads[-1], eps=1e-6)
    ...
    def forward_features(self, x):
        ...
        upperAU_feat = self.norm(self.gap_upperAU(upper_AU).squeeze())
        lowerAU_feat = self.norm(self.gap_lowerAU(lower_AU).squeeze()) 
        ...
        return cls_features,upperAU_feat,lowerAU_feat

    def forward(self, x):
        cls_features,upperAU_feat,lowerAU_feat = self.forward_features(x)
        output = self.head(cls_features)
        upperAU = self.upperAU_head(upperAU_feat)
        lowerAU = self.lowerAU_head(lowerAU_feat)
        AU_output=torch.cat((upperAU, lowerAU), dim=1) #just for visualization
