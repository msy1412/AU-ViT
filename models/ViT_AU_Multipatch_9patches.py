...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',AU_num_classes=21,supervise_layer=10,num_AU_patch=9):
        """
        Args:
        ...
        if num_AU_patch==9:
            ...
            self.norm_AU31  = nn.LayerNorm(embed_dim, eps=1e-6)
            self.norm_AU32  = nn.LayerNorm(embed_dim, eps=1e-6)
            self.norm_AU33  = nn.LayerNorm(embed_dim, eps=1e-6)
            ...
            self.head_AU31= nn.Linear(embed_dim, 14)
            self.head_AU32= nn.Linear(embed_dim, 14)
            self.head_AU33= nn.Linear(embed_dim, 14)
        elif num_AU_patch==7:
            self.gap_AU11 = nn.AdaptiveAvgPool2d(1)
            self.gap_AU12 = nn.AdaptiveAvgPool2d(1)
            self.gap_AU13 = nn.AdaptiveAvgPool2d(1)
