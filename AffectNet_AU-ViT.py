...
def parse_arguments():
    ...
    parser.add_argument('--AU_patch', type=int, default=7)
    parser.add_argument('--print_freq', type=int, default=100)
    parser.add_argument('--valid_freq', type=int, default=700)
    ...
    parser.add_argument('--learning_rate', type=float, default=5e-5,
                        help='learning rate') 
    parser.add_argument('--weight_decay', type=float, default=5e-3,#must have
                        help='weight decay')
    parser.add_argument('--momentum', type=float, default=0.9,
                        help='momentum')
    parser.add_argument('--scheduler', type=str ,default="cosin",
                        help='scheduler: cosin or step or none')
    parser.add_argument('--iter_cosin', type=bool,default=True,
                        help='cosin decay each iteration')
...
def set_loader(args):
    ...
    if args.dataset == 'AffectNet':
        ...
        AU_test_loader = data.DataLoader(AU_test_dataset,batch_size=32, shuffle=True,num_workers=args.num_workers, pin_memory=True)

        TrainlabelPath = parameter.AffectNet_train_npy
        TestlabelPath = parameter.AffectNet_validation_npy
        FER_img_folder_path =parameter.AffectNet_img_path
        img_folder=parameter.AffectNet_train_folder_7

        TrainDataset=datasets.ImageFolder(img_folder, train_transform)
        # TrainDataset=AffectNet.load_AffectNet(TrainlabelPath,FER_img_folder_path,transform=train_transform)
        # TrainLoader=data.DataLoader(TrainDataset,batch_size=args.batch_size,num_workers=args.num_workers,shuffle=True)
        TrainLoader = data.DataLoader(TrainDataset, sampler=sample.BalancedBatchSampler(TrainDataset), batch_size=args.batch_size,num_workers=args.num_workers)
        
        TestDataset=AffectNet.load_AffectNet_7(TestlabelPath,FER_img_folder_path,transform=val_transform)
        TestLoader=data.DataLoader(TestDataset,batch_size=args.batch_size,num_workers=args.num_workers,shuffle=True)
...
def set_model(args):
    ...
    model =IR_RVT_AU_plus_patch.IR50_ViT(num_classes=7,ir_50_pth=pthpath,num_AU_patch=args.AU_patch)
...
def train(AU_train_loader,FER_train_loader, AU_test_loader,FER_val_loader,model, criterion, optimizer,warmup_scheduler, epoch, warmup_epoch,scheduler, args):
    """one epoch training"""
    ...
    for idx, (images, FER_targets) in enumerate(FER_train_loader):
        ...
        loss = FER_RATIO * loss_EXPR + AU_RATIO * loss_AU
        ...
        elif args.scheduler and args.iter_cosin==True :
        ...
        if (idx + 1) % args.valid_freq == 0:
            ...
            print('Validation epoch {}, total time {:.2f}, EXPR F1:{:.4f}, accuracy:{:.4f}, total accuracy:{:.4f}, AU F1:{:.4f}, accuracy:{:.4f}, total accuracy:{:.4f}'.format(
                epoch, time4 - time3, EXPR_accs[0], EXPR_accs[1], EXPR_accs[2], AU_accs[0], AU_accs[1], AU_accs[2]))
            if args.save_model:
                if EXPR_accs[1]>= 65:
                    print("save weights")
...
def main():
    ...
    if args.scheduler == 'step':
        ...
        if args.iter_cosin==True:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs *len(FER_train_loader),eta_min=5e-7)
        else:
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs - warmup_epoch-1,eta_min=5e-7)
    ...
    for epoch in range(1, args.epochs + 1):
        # train for one epoch
        time1 = time.time()
        loss, EXPR_accs, AU_accs = train(AU_train_loader,FER_train_loader,AU_test_loader,FER_val_loader, model, criterion, optimizer,warmup_scheduler, epoch,warmup_epoch,scheduler, args)
        time2 = time.time()
        ...
        if args.scheduler and args.iter_cosin==False:
            if args.warmup and epoch < warmup_epoch:
                continue
            scheduler.step()  # update lr
